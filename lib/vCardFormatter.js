...
(function vCardFormatter() {
  ...
  */
  function getFormattedAddress(address) {
    ...
    if( address && ( address.label || address.street || address.city || address.stateProvince || address.postalCode || address.countryRegion ) ){
  ...
  module.exports = {
    ...
    */
    getFormattedString: function(vCard) {
      ...
      if (!formattedName && vCard.name) {
        formattedName = '';

        [vCard.name.first, vCard.name.middle, vCard.name.last]
      ...
      if(vCard.name && typeof vCard.name === 'object' ){
        formattedVCardString += getFormattedName(vCard.name);
      }
      ...
      if( vCard.email && typeof vCard.email === 'object' ){

        vCard.email = (vCard.email.length !== null) ? vCard.email : [vCard.email];
        vCard.email.forEach(function(email) {

          if(!Object.keys(email)) return;

          if (majorVersion >= 4) {
            formattedVCardString += 'EMAIL:' + e(email) + nl();
          } else {
            formattedVCardString += 'EMAIL;PREF;INTERNET:' + e(email) + nl();
          }
